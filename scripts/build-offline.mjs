...
import {MODEL_REVISION} from '../src/revision.js';
const result=await build({entryPoints:['src/main.js'],bundle:true,minify:true,write:false,format:'iife',target:'es2022',loader:{'.css':'empty'},legalComments:'inline'});
const textures={};
for(const f of await readdir('public/textures'))if(f.endsWith('.png'))textures[f]='data:image/png;base64,'+(await readFile('public/textures/'+f)).toString('base64');
...
html=html.replace('</head>',()=>`<style>${css}</style><meta name="wiring-export" content="${MODEL_REVISION}"></head>`)
 .replace('<script type="module" src="/src/main.js"></script>',()=>`<script>${dataScript}</script><script>${bundle}</script>`);
...
for(const s of scripts)new Script(s[2]);
