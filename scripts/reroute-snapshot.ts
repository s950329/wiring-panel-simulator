...
export function createRuntime() {
    ...
    return { ...model, routing, simulation, page: 'board' as const };
}
/** Explicit offline reroute; ordinary snapshot import continues to preserve saved geometry. */
export function rerouteSnapshot(source: string, { closePanel = false, progress = () => { } }: {closePanel?: boolean; progress?: (id: string) => void} = {}) {
    ...
    for (const wire of topology) {
        ...
        try {
            routing.connect(wire.from, wire.to);
        }
        catch (error) {
            throw new Error(`${wire.id} (${wire.from.component}:${wire.from.terminal} → ${wire.to.component}:${wire.to.terminal}): ${error instanceof Error ? error.message : String(error)}`, { cause: error });
        }
        progress(wire.id);
    }
    ...
    if (closePanel && view.operationPanelOpen) {
        ...
        view.operationPanelOpen = false;
        view.panelAngle = 0;
    }
    routing.select(null);
    for (const c of components.values())
        c.updateView(components.get(c.placement.parentId ?? ""), true);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
    ...
    if (!input || !output || (option && option !== '--close-panel') || process.argv.length > 5) {
        console.error('Usage: node --import tsx scripts/reroute-snapshot.ts INPUT.json OUTPUT.json [--close-panel]');
        process.exitCode = 1;
    }
