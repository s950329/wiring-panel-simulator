import type {
  ...
  ComponentRuntime, ComponentState, ComponentView, ResolvedComponent, StatusContext,
} from './contracts.ts';
...
export class ComponentInstance<S extends ComponentState, A extends ComponentAction> implements ComponentRuntime {
  ...
  get pressed(): boolean { return this.#state.kind === 'momentary' && this.#state.pressed; }
  get audible(): boolean { return this.behavior.audible?.(this.#state) ?? false; }
  ...
  present(context: StatusContext = {connections: 0}) { return this.behavior.present(this.#state, context); }
  updateView(parent?: ComponentRuntime, instant = false): void {
    this.view.update(this.#state, {parentPressed: parent?.pressed ?? false}, instant);
  }
