...
function convertSnapshot(value:Record<string,unknown>):ProjectDocument{
 ...
 for(const [id,definitionId,kind]of [['CONTROL','teaching-source','control'],['MAIN','teaching-three-phase-source','main'],['M1','teaching-motor',null]] as const){
  requireField(!ids.has(id),id,'舊格式外接設備 ID 與盤內元件衝突');
  components.push({id,definitionId,definitionVersion:1,placement:null,...(kind?{parameters:{enabled:boolean(power[kind],`simulation.power.${kind}`)}}:{})});ids.add(id);
 }
 ...
 const project:ProjectDocument={format:'wiring-panel-project',schemaVersion:1,name:'匯入的配線專案',configuration:{
  ...
  connections:[...physical,...external].map((v,i)=>oldConnection(v,`connections[${i}]`))};
...
export function readProject(source:string):{project:ProjectDocument;convertedLegacy:boolean}{
 const value=record(safeJSON(source),'檔案');
 if(value.format==='wiring-panel-project')return{project:validateProject(value),convertedLegacy:false};
 if(value.format==='wiring-panel-snapshot')return{project:convertSnapshot(value),convertedLegacy:true};
