...
export function startProjectApp(){
 ...
 function installRuntime(){
  ...
  currentId=active().components.keys().next().value??null;$('#project-name').value=active().project.name||'';$('#project-meta').toggleAttribute('data-i18n-ignore',!!active().project.name);$('#project-meta').textContent=active().project.name||'配線專案';
 ...
 $('#project-name').onchange=()=>{if(session.busy)return;const name=$('#project-name').value.trim();if(name)active().project.name=name;else delete active().project.name;$('#project-meta').toggleAttribute('data-i18n-ignore',!!name);$('#project-meta').textContent=name||'配線專案';};
