...
export function createScene(container,{inspectMC1=false,model=null}={}){
...
function resize(){let r=container.getBoundingClientRect();if(!r.width||!r.height)return;renderer.setSize(r.width,r.height);camera.aspect=r.width/r.height;camera.updateProjectionMatrix();orbit.fitRadius=inspectMC1?Math.max(350,155/(Math.tan(20*Math.PI/180)*camera.aspect)):Math.max(1060,480/(Math.tan(20*Math.PI/180)*camera.aspect))*(boardSize?Math.max(boardSize.width/800,boardSize.depth/640,.2):1);if(!orbit.hasResized||r.width<700){orbit.radius=orbit.fitRadius;orbit.update();}orbit.hasResized=true;}
const resizeObserver=new ResizeObserver(resize);resizeObserver.observe(container);resize();if(inspectMC1)orbit.preset('perspective');
...
let flapTarget=flap.rotation.x,frame=0,disposed=false;function setFlap(open,instant=false){flapTarget=open?Math.PI:0;if(instant){flap.rotation.x=flapTarget;world.updateMatrixWorld(true);}}
function draw(){if(disposed)return;flap.rotation.x+=(flapTarget-flap.rotation.x)*.13;for(const c of components.values())c.updateView(components.get(c.def.parentId));
if(selectedTerminal){world.updateMatrixWorld(true);selectedTerminal.getWorldPosition(terminalGlow.position);selectedTerminal.getWorldQuaternion(terminalGlow.quaternion);terminalGlow.quaternion.multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(1,0,0),-Math.PI/2));terminalGlow.position.add(new T.Vector3(0,4,0).applyQuaternion(selectedTerminal.getWorldQuaternion(new T.Quaternion())));}
if(selected){visibleBounds(selected.root,highlight.box);highlight.box.expandByScalar(3);}renderer.render(scene,camera);frame=requestAnimationFrame(draw);}draw();
...
function fitProject(){
 ...
 orbit.minRadius=80;orbit.maxRadius=Math.max(2400,span*6);camera.far=Math.max(5000,span*12);camera.updateProjectionMatrix();
 grid.scale.set(boardSize.width/800,1,boardSize.depth/800);ground.scale.set(Math.max(1,size/800),Math.max(1,size/800),1);ground.position.y=-Math.max(22,boardSize.thickness+15);
 const factor=Math.max(1,span/800);sun.position.set(-350*factor,900*factor,100*factor);Object.assign(sun.shadow.camera,{left:-680*factor,right:680*factor,top:680*factor,bottom:-680*factor,far:1800*factor});sun.shadow.camera.updateProjectionMatrix();
 orbit.hasResized=false;resize();orbit.preset('perspective');
}
function setProject(next){
 if(!binding)throw new Error('舊版檢視不可切換專案');select(null);binding.replace(next);world=binding.world;components=binding.components;flap=binding.flap||new T.Group();boardSize=next.configuration.board;flapTarget=flap.rotation.x;
 fitProject();
}
function inspect(id){if(!binding)return;binding.inspect(id);if(id)focus(id);else orbit.preset('perspective');}
function dispose(){if(disposed)return;disposed=true;cancelAnimationFrame(frame);resizeObserver.disconnect();highlight.geometry.dispose();highlight.material.dispose();terminalGlow.geometry.dispose();terminalGlow.material.dispose();grid.geometry.dispose();grid.material.dispose();ground.geometry.dispose();ground.material.dispose();scene.environment?.dispose();renderer.dispose();renderer.domElement.remove();}
fitProject();
...
export function buildModel(scene,{inspectMC1=false}={}){
 if(inspectMC1){const world=new T.Group();scene.add(world);world.userData.routingContext={ducts:ducts.map((d,i)=>({...d,y:.5,id:`duct-${i+1}`,tag:i})),gateway:panelGateway};const origin=layout.find(d=>d.id==='MC1');world.position.set(-origin.x,0,-origin.z);const components=mountComponents(world,layout.filter(d=>d.id==='MC1'||d.parentId==='MC1'));return {world,components,flap:new T.Group()};}

const world=new T.Group();scene.add(world);world.userData.routingContext={ducts:ducts.map((d,i)=>({...d,y:.5,id:`duct-${i+1}`,tag:i})),gateway:panelGateway};world.position.set(-400,0,-320);
...
for(const c of components.values())if(c.definition.behavior==='lamp'&&c.parts.color)c.parts.color.envMap=scene.environment;
