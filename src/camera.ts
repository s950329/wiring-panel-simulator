...
export class HemisphereCamera {
    ...
    fitRadius?: number;
    hasResized?: boolean;
    constructor(camera: T.PerspectiveCamera) { this.camera = camera; this.azimuth = DEFAULT_AZIMUTH; this.elevation = 1.03; this.radius = 1060; this.target = new T.Vector3(0, 0, 5); this.update(); }
    ...
    preset(name: string) { this.target.set(0, 0, 5); this.azimuth = DEFAULT_AZIMUTH; this.radius = this.fitRadius || 1060; this.elevation = name === 'top' ? Math.PI / 2 : name === 'side' ? 0 : 1.03; this.update(); }
}
