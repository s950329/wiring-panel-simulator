import type {ModelContext, TerminalView} from '../core/contracts.ts';
import * as T from 'three';
import { mat, box, cyl, hit, label, terminal } from '../primitives.ts';
...
function profile(root: T.Object3D, x: number, thickness: number, points: readonly (readonly [number, number])[], material: T.Material = mat.black) {
    ...
    mesh.position.x = x + thickness / 2;
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    root.add(mesh);
    return mesh;
}
function contact(c: ModelContext, id: string, x: number, y: number, z: number, metadata: Partial<Pick<TerminalView, "displayName" | "group">> & {side?: string; end?: string; level?: string} = {}) {
    ...
    Object.assign(c.terminals.at(-1)!, metadata);
    return g;
}
export function buildMC1(c: ModelContext): ModelContext {
    const g = c.root;
    ...
    for (const x of [-33, 33])
        box(g, 3, 38, 69, x, 48, 0, mat.dark, .5);
    ...
    const rearMark = label(g, 'A1          A2', 37, 7, 11, 36, -57.2, { bg: '#242824', fg: '#866a59', size: 27 });
    rearMark.rotation.set(0, Math.PI, 0);
    for (const t of MC1_GEOMETRY.rearLower) {
        box(g, 18, 10, 24, t.x, t.y - 7, t.z, mat.black, .7);
        for (const dx of [-9, 9])
            box(g, 2, 21, 26, t.x + dx, t.y - 2, t.z, mat.dark, .4);
