...
export function createWirePanel(app,{toast,isFlapOpen,onChange,simulation=()=>null,runtime=null}){
 let disposed=false;
 const editable=()=>!disposed&&(!runtime||!runtime.locked)&&(!simulation()||simulation().canEdit);
 const isExternal=id=>simulation()?.isExternal?simulation().isExternal(id):isExternalEquipment(id);
 ...
 function render(message){
  ...
  $('[data-wire-mode="connect"]').disabled=!editable()||busy;$('[data-wire-mode="operate"]').disabled=busy||!!runtime?.locked;
 ...
 function removeWire(id){if(busy||!editable())return;try{if(runtime)runtime.remove(id);else if(id.startsWith('E'))simulation().removeExternal(id);else routing.remove(id);history=history.filter(entry=>entry!==id);externalSelected=null;onChange?.();render();}catch(e){toast(e.message);}}
 function cancel(){if(disposed)return;pending=null;render();}
 function select(id){
  if(disposed)return;
  evidence.clear();
 ...
 const onKey=e=>{
  if(disposed||runtime?.locked)return;
 ...
 return {element:panel,dispose(){if(disposed)return;disposed=true;pending=null;window.removeEventListener?.('keydown',onKey);panel.remove?.();},routing,pick,select,render,setMode,cancel,trace,clearEvidence,isConnect:()=>mode==='connect'&&editable(),isBusy:()=>busy,
  ...
  movePanel(open,previous){if(runtime){runtime.movePanel(open);app.setFlap(open,true);}else routing.movePanel(()=>app.setFlap(open,true),()=>app.setFlap(previous,true),front);if(editable())setMode(open?'connect':'operate');},
