...
export function createWirePanel(app,{toast,isFlapOpen,onChange,simulation=()=>null}){
 ...
 function render(message){
  ...
  $('[data-wire-all]').disabled=!routing.selected&&!externalSelected;
 ...
 async function pick(component,terminal){
  ...
  try{
   ...
   history.push(w.id);externalSelected=w.id.startsWith('E')?w.id:null;if(externalSelected)routing.select(null);
 ...
 $('[data-wire-all]').onclick=()=>{routing.select(null);externalSelected=null;render();};
 ...
 return {routing,pick,select,render,setMode,cancel,isConnect:()=>mode==='connect'&&editable(),isBusy:()=>busy,
