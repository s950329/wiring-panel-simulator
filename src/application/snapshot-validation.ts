...
const compatibleRevisions = new Set(['WIRE-R8', 'WIRE-R9', 'WIRE-R10', 'WIRE-R11']);
...
export function parseBoardSnapshot(source: string, components: ReadonlyMap<string, ComponentRuntime>, page: BoardViewState['page']) {
  ...
  requireValue(typeof data.revision === 'string' && compatibleRevisions.has(data.revision), '模型版本不相容，支援 WIRE-R8～WIRE-R11 匯出的檔案');
