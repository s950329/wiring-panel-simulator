...
def run(browser, page):
    ...
    ready(page)
    check(page.evaluate('window.wiringLab.getRevision()') == 'WIRE-R19', 'R19 boot')
    ...
    for url, label in [(BASE + '/wiring-panel.html', 'served standalone'),
                       ((ROOT / 'downloads/wiring-panel-WIRE-R19.html').as_uri(), 'file-protocol standalone')]:
        ...
        ready(offline)
        check(offline.evaluate("window.wiringLab.getRevision()==='WIRE-R19' && window.wiringLab.getProject().format==='wiring-panel-project'"), label + ' boot')
