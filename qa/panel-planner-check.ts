...
import type {PlanDiagnostics} from '../src/wiring/planner.ts';
import {describeTerminal, routeWire, validateSelf} from '../src/wiring/router.ts';
import {collectSolids} from '../src/wiring/solids.ts';
import {required} from './helpers/fixture-types.ts';
import {panelBacksideGeometry} from './helpers/panel-space-oracle.ts';
...
const connection = ([a, at, b, bt]: readonly [string,string,string,string]) => ({ from: endpoint(a, at), to: endpoint(b, bt) });
const uploaded: ProjectDocument = JSON.parse(readFileSync(new URL('./fixtures/panel-closure-11.project.json', import.meta.url), 'utf8'));
const classroom: ProjectDocument = JSON.parse(readFileSync(new URL('../examples/board-024-classroom.project.json', import.meta.url), 'utf8'));
const r8 = defaultProject();
required(r8.configuration.operationPanel).state.open = true;
r8.connections = ([
...
] as const).map(connection);
function fresh(source: string|ProjectDocument) {
    const project = readProject(JSON.stringify(source)).project;
    project.connections = [];
    required(project.configuration.operationPanel).state.open = true;
    return createProjectRuntime(project);
}
function geometry(runtime: ProjectRuntime, label = '') {
    ...
    for (const wire of runtime.routing.wires) {
        ...
        for (const [end, point] of [[wire.from, wire.points[0]], [wire.to, wire.points.at(-1)]] as const) {
            const anchors = describeTerminal(runtime.world, runtime.components, end).anchors;
            assert.ok(anchors.some(p => Math.hypot(...p.map((n, i) => n - required(point)[i])) < .002), `${label}: ${wire.id} detached from ${required(end).component}:${required(end).terminal}`);
        }
        ...
        if (runtime.front.has(wire.from.component) || runtime.front.has(wire.to.component)) {
            ...
            assert.ok(wire.points.every((p: readonly number[]) => p[2] >= 517 - 1e-6), `${label}: ${wire.id} leaves the panel side`);
        }
        accepted.push(wire);
    }
...
function identities(runtime: ProjectRuntime) {
    return runtime.routing.wires.map(({ id, from, to }) => ({ id, from, to }));
}
function poses(runtime: ProjectRuntime, label: string|Error|undefined) {
    ...
    for (const open of [false, true]) {
        ...
        assert.equal(required(runtime.flap).rotation.x, open ? Math.PI : 0, label);
...
const variants: Record<string, (wires: readonly import('../src/project/contracts.ts').Connection[]) => import('../src/project/contracts.ts').Connection[]> = {
    ...
    interleaved: (wires) => [...wires.filter((_, i: number) => i % 2), ...wires.filter((_, i: number) => !(i % 2))]
        .map(({ from, to }, i) => i % 2 ? { from: { ...from }, to: { ...to } } : { from: { ...to }, to: { ...from } }),
};
for (const [name, source] of [['uploaded-11', uploaded], ['classroom-22', classroom], ['R8-full-23', r8]] as const) {
    ...
    test(`${name}: repeated cycles, delete/reconnect and closed project round trip retain the full topology`, async () => {
        ...
        let restored;
        try {
            for (const wire of source.connections)
                runtime.connect(wire.from, wire.to);
            for (let cycle = 0; cycle < 3; cycle++)
                poses(runtime, `${name}/cycle-${cycle}`);
            ...
            assert.ok(selected);
            const oldIds = identities(runtime).filter((w: { id: string; }) => w.id !== selected.id);
            ...
            assert.deepEqual(identities(runtime).filter((w: { id: string; }) => w.id !== replacement.id), oldIds);
            poses(runtime, `${name}/reconnected`);
            runtime.movePanel(false);
            const exported = runtime.exportProject();
            const rebuildPlans: PlanDiagnostics[] = [], originalPlan = WiringController.prototype.plan;
            ...
            try {
                ...
                WiringController.prototype.plan = originalPlan;
            }
            if (name === 'R8-full-23') {
...
test('a new wire that cannot operate in the opposite pose is rejected atomically', () => {
    ...
    try {
        ...
        assert.equal(required(runtime.flap).rotation.x, Math.PI);
...
test('cached feasible poses are revalidated against new physical obstacles, with full movement rollback', () => {
    const runtime = fresh(uploaded), obstacle = blockedTerminal();
    try {
        for (const wire of uploaded.connections)
            runtime.connect(wire.from, wire.to);
...
test('an unrelated cabinet wire does not change an unoccupied panel route candidate', () => {
    ...
    try {
        const ends: [import('../src/electrical/contracts.ts').Endpoint,import('../src/electrical/contracts.ts').Endpoint] = [endpoint('TB1', '46B'), endpoint('HL3', '2')];
        const empty = routeWire(runtime.world, runtime.components, ...ends, []);
        const cabinet = {...routeWire(runtime.world, runtime.components, endpoint('QF1', 'T1'), endpoint('FU1', 'F1-IN'), []),id:'cabinet'};
        assert.ok(new CollisionWorld([], [{...cabinet,id:'cabinet'}]).validate(empty.points), 'cabinet wire is physically independent');
        ...
        assert.deepEqual(routeWire(runtime.world, runtime.components, ...ends, emptyRecords).points, empty.points, 'records with no physical segments cannot allocate height or lane slots');
    }
    finally {
        runtime.dispose();
    }
