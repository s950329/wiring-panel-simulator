...
const module = await import('../src/project/validation.ts').catch(()=>({}));
const parse = source => {assert.equal(typeof module.parseProject,'function','project parser must exist');return module.parseProject(source);};
const load = p => parse(JSON.stringify(p));
...
test('project validation rejects unknown fields, versions, prototypes, bad values and unresolved mounts',()=>{
  for(const mutate of [p=>p.schemaVersion=99,p=>p.configuration.components[0].definitionVersion=2,
    p=>p.configuration.components[0].definitionId='remote-model',p=>p.configuration.board.width=-1,
    p=>p.configuration.components[0].placement.rotationY=45,p=>p.configuration.components[0].placement.mountId='missing',
    p=>p.configuration.components[0].state.on='true',p=>p.configuration.components[0].points=[],
    p=>p.configuration.components.push({...p.configuration.components[0]}),p=>p.configuration.board.extra='wrong']){
    const p=minimalProject();mutate(p);assert.throws(()=>load(p));
  }
...
test('assembly slots enforce type, unique occupancy and explicit fixed conductors',()=>{
  const good=withAssembly();assert.equal(load(good).configuration.components.length,4);
  for(const mutate of [p=>p.configuration.assemblies[0].hostId='missing',p=>p.configuration.assemblies[0].hostId='thermal',
    p=>p.configuration.components[2].placement.slot='missing',p=>p.configuration.components[2].definitionId='shihlin-th20',
    p=>p.configuration.components[3].parameters.current=3.3,p=>p.configuration.components[3].parameters.current=15.1,
    p=>p.configuration.components.push({...p.configuration.components[2],id:'second-aux'})]){
    const p=withAssembly();mutate(p);assert.throws(()=>load(p));
  }
  good.connections=[{from:ep('coil','2T1'),to:ep('thermal','1/L1')}];assert.throws(()=>load(good),/固定|重複/);
});
test('connections validate undirected duplication and endpoints but permit distinct branches and deliberate electrical faults',()=>{
  const p=minimalProject();p.connections=[{from:ep('breaker','T1'),to:ep('coil','1L1')},
    {from:ep('breaker','T1'),to:ep('coil','3L2')}];assert.equal(load(p).connections.length,2);
  p.connections.push({from:ep('coil','1L1'),to:ep('breaker','T1')});assert.throws(()=>load(p),/重複/);
  p.connections=[{from:ep('coil','A1'),to:ep('coil','A1')}];assert.throws(()=>load(p),/自接|相同/);
...
test('resource limits are enforced before model construction and transient inputs cannot become saved outputs',()=>{
  const p=minimalProject();p.configuration.components[1].state={pressed:true};
  const out=load(p);assert.equal(out.configuration.components[1].state?.pressed,undefined);
  p.configuration.components[1].state={energized:true};assert.throws(()=>load(p),/state|欄位/);
  p.configuration.components=[];p.connections=Array.from({length:513},()=>({from:ep('x','1'),to:ep('y','2')}));assert.throws(()=>load(p),/connections/);
