...
test('default project rebuild retains every accepted terminal geometry with configured equipment',()=>{
  ...
  assert.equal(m.components.size,22);assert.equal(m.simulation.equipment.length,2);
...
test('identical renamed hosts share a source but an actual branch contact controls only its wired load',()=>{
  const p=minimalProject();p.configuration.components[0].state={on:false};p.configuration.components.push(
    {...p.configuration.components[1],id:'second',placement:{mountId:'base',position:[356,7,280],rotationY:-90}},
    {id:'supply-X',definitionId:'teaching-ac220-three-phase-source',definitionVersion:1,placement:null});
  const m=make(p);
  m.connect(ep('supply-X','L1'),ep('coil','A1'));m.connect(ep('supply-X','L3'),ep('coil','A2'));
  m.connect(ep('supply-X','L1'),ep('breaker','L1'));m.connect(ep('breaker','T1'),ep('second','A1'));m.connect(ep('supply-X','L3'),ep('second','A2'));
  const r=m.simulation.start();assert.equal(r.status,'stable');assert.equal(r.coils.coil,true);assert.equal(r.coils.second,false);
  m.simulation.operate('breaker',{type:'toggle'});assert.equal(m.simulation.snapshot().result.coils.second,true);
  m.simulation.operate('breaker',{type:'toggle'});assert.equal(m.simulation.snapshot().result.coils.coil,true);assert.equal(m.simulation.snapshot().result.coils.second,false);m.dispose();
});
