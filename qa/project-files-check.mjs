...
class Element{children=[];dataset={};disabled=false;hidden=false;value='';files=[];clicked=0;textContent='';append(...n){this.children.push(...n)}prepend(...n){this.children.unshift(...n)}setAttribute(){}click(){this.clicked++}remove(){this.removed=true}}
function make(overrides={}){globalThis.document={createElement:()=>new Element()};const events=[];
...
test('oversized, read and reconstruction errors preserve the original and remain visible',async()=>{
 ...
 ui.input.files=[{size:11*1024*1024,text:async()=>{throw Error('must not read')}}];await ui.input.onchange();assert.equal(called,0);assert.match(ui.status.textContent,/10 MB/);
 ui.input.files=[{size:20,text:async()=>'{}'}];await ui.input.onchange();assert.equal(called,1);assert.match(ui.status.textContent,/第 2 條.*原專案/);assert.equal(ui.importButton.disabled,false);
 ui.input.files=[{size:20,text:async()=>{throw Error('read failed')}}];await ui.input.onchange();assert.match(ui.status.textContent,/read failed/);assert.equal(ui.input.value,'');
});
test('normal download contains only project inputs and sanitized filename; debug export is a separate format',()=>{
 assert.equal(typeof files.projectDownload,'function');const project={format:'wiring-panel-project',schemaVersion:1,name:'../../<Test>',configuration:{},connections:[{from:{component:'a',terminal:'1'},to:{component:'b',terminal:'2'}}]};
 const file=files.projectDownload(project);assert.deepEqual(JSON.parse(file.content),project);assert.ok(!file.filename.includes('/'));assert.ok(file.filename.endsWith('.json'));assert.equal(file.mimeType,'application/json');
});
