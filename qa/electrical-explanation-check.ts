...
const line = (id: string, a: readonly [string, string], b: readonly [string, string]) => ({ id, from: ep(...a), to: ep(...b) });
const coil = (entries: ReturnType<typeof explainSimulation>) => required(entries.find(e => e.id === 'load:MC1:coil'));
test('powered explanations trace actual conducting nets without crossing the coil load', () => {
    const c = mutableCircuit(minimalControlCircuit()), r = settleCircuit(c, { PB1: { pressed: true } });
    const t = traceEndpoint(c, required(r.evaluation), ep('MC1', 'A1'));
    assert.deepEqual(t.sources, [ep('SUPPLY', 'L')]);
    assert.ok(t.wireIds.includes('feed'));
    assert.ok(t.wireIds.includes('start'));
    assert.equal(t.wireIds.includes('return'), false);
    assert.equal(t.endpoints.some(e => e.component === 'MC1' && e.terminal === 'A2'), false);
    const e = coil(explainSimulation(c, r));
    assert.match(e.title, /吸合/);
    assert.match(e.detail, /相符/);
    assert.ok(e.wireIds.includes('return'));
    assert.equal(e.endpoints.length, 2);
});
test('open contacts are stated as observed evidence, not a unique wiring mistake', () => {
    const c = mutableCircuit(minimalControlCircuit()), r = settleCircuit(c);
    const e = coil(explainSimulation(c, r));
    assert.match(e.title, /未吸合/);
    assert.match(e.detail, /完整回路/);
    assert.ok(e.openContacts.some((contact: { component: string; id: string; }) => contact.component === 'PB1' && contact.id === 'NO'));
    assert.equal(e.openContacts.some((contact: { id: string; }) => contact.id === 'NC'), false);
    assert.match(e.detail, /保持/);
    assert.doesNotMatch(e.detail, /一定|唯一|接錯了/);
});
test('missing return and same-potential loads expose the affected endpoints', () => {
    const c = mutableCircuit(minimalControlCircuit()), missing = { ...c, wires: c.wires.filter(w => w.id !== 'return') };
...
test('motor explanations distinguish incomplete phases from an energized contactor', () => {
    ...
    const missing = { ...c, wires: c.wires.filter(w => w.id !== 'motor-W') };
    const r = settleCircuit(missing, { QF1: { on: true }, PB3: { pressed: true } }), es = explainSimulation(missing, r);
    assert.match(coil(es).title, /吸合/);
    const m = es.find(e => e.id === 'motor:M1:motor');
    assert.match(required(m).title, /缺相/);
    assert.ok(required(m).traces.some(t => t.endpoint.terminal === 'W' && !t.sources.length));
    assert.match(required(m).detail, /三個不同相別/);
});
test('source-off and idle unconnected loads do not flood the user with unrelated errors', () => {
    ...
    c.components.push(mutableClone(createTeachingComponent({ id: 'HL1', definitionId: 'lamp-white' })));
    const off = { ...c, sources: c.sources.map(s => ({ ...s, enabled: false })) };
    const es = explainSimulation(off, settleCircuit(off));
    assert.match(coil(es).title, /電源未開啟/);
...
test('short circuit and unsupported terminal explanations keep fault endpoints without partial output', () => {
    ...
    const unknown = { ...c, components: [...c.components, mutableClone(createTeachingComponent({ id: 'UNVERIFIED', definitionId: 'cn18' }))], wires: [...c.wires, line('unknown', ['SUPPLY', 'L'], ['UNVERIFIED', '21NC'])] };
    const e = explainSimulation(unknown, settleCircuit(unknown)).find(e => e.id.startsWith('diagnostic:MISSING_MODEL'));
    assert.match(required(e).detail, /尚未確認/);
    assert.ok(required(e).endpoints.some(e => e.component === 'UNVERIFIED' && e.terminal === '21NC'));
});
test('oscillation and non-convergence have actionable stop/edit/retry explanations', () => {
    const c = mutableCircuit(minimalControlCircuit());
    c.components.push(mutableClone(createTeachingComponent({ id: 'AP1', definitionId: 'shihlin-ap22', parentId: 'MC1' })));
    ...
    assert.match(required(e).title, /反覆切換/);
    assert.match(required(e).detail, /停止模擬/);
    const minimal = mutableCircuit(minimalControlCircuit());
    const limited = settleCircuit(minimal, { PB1: { pressed: true } }, {}, { maxIterations: 1 });
...
test('evidence is stable under wire direction and order changes and does not mutate inputs', () => {
    const c = mutableCircuit(directOnLineCircuit()), before = structuredClone(c), r = settleCircuit(c, { QF1: { on: true }, PB3: { pressed: true } });
