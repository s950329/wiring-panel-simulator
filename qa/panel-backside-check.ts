...
import {describeTerminal, validateSelf} from '../src/wiring/router.ts';
import {collectSolids} from '../src/wiring/solids.ts';
import {required} from './helpers/fixture-types.ts';
import {panelBacksideGeometry, routeLength} from './helpers/panel-space-oracle.ts';
...
function physicalGeometry(runtime: ProjectRuntime) {
    ...
    for (const wire of runtime.routing.wires) {
        ...
        for (const [end, point] of [[wire.from, wire.points[0]], [wire.to, wire.points.at(-1)]] as const)
            assert.ok(describeTerminal(runtime.world, runtime.components, end).anchors.some(anchor => Math.hypot(...anchor.map((n, i) => n - required(point)[i])) < .002), `${wire.id} endpoint`);
        accepted.push(wire);
    }
}
test('reported eleven-wire circuit stays behind the panel throughout repeated working poses', async () => {
    ...
    try {
        ...
        for (const open of [false, true, false, true, false]) {
            ...
            panelBacksideGeometry(runtime, open ? 'open' : 'closed');
            physicalGeometry(runtime);
...
test('a cached collision-free R17 route across the operation face is replaced before closing', () => {
    ...
    required(source.configuration.operationPanel).state.open = true;
    ...
    try {
        ...
        required(runtime.flap).rotation.x = 0;
        runtime.routing.syncPose();
        assert.ok(validateSelf(bad.points));
        assert.ok(new CollisionWorld(collectSolids(runtime.world)).validate(bad.points));
        for (const [end, point] of [[bad.from, bad.points[0]], [bad.to, bad.points.at(-1)]] as const)
        ...
        panelBacksideGeometry(runtime, 'replacement cache');
        physicalGeometry(runtime);
...
test('an isolated same-panel lamp-to-button wire uses a compact backside connection in both directions', () => {
    for (const reversed of [false, true]) {
        const source = defaultProject();
        required(source.configuration.operationPanel).state.open = true;
        const runtime = createProjectRuntime(source), ends: [import('../src/electrical/contracts.ts').Endpoint,import('../src/electrical/contracts.ts').Endpoint] = [ep('HL3', '2'), ep('PB5', '4')];
        if (reversed)
            ends.reverse();
        try {
            ...
            for (const open of [false, true]) {
                runtime.movePanel(open);
                const wire = runtime.routing.wires[0];
                panelBacksideGeometry(runtime, 'isolated same-panel');
                physicalGeometry(runtime);
                const shortestOrthogonal = wire.points[0].reduce((sum, n, i) => sum + Math.abs(n - required(wire.points.at(-1))[i]), 0);
                assert.ok(routeLength(wire) <= shortestOrthogonal + 60, `unoccupied neighboring terminals should not detour around the outer frame: ${routeLength(wire)} versus ${shortestOrthogonal}`);
            }
        }
        finally {
            runtime.dispose();
...
function transformedPanel(side: 'A' | 'B', rotation: number) {
    ...
    const transform = ([x, y, z]: Vec3): [number,number,number] => [400 + (x - 400) * Math.cos(angle) + (z - 320) * Math.sin(angle) + 17,
        y + 23, 320 - (x - 400) * Math.sin(angle) + (z - 320) * Math.cos(angle) - 11];
    required(cfg.operationPanel).position = transform(required(cfg.operationPanel).position);
    required(cfg.operationPanel).rotationY = rotation;
    const gateway = cfg.components.find(c => c.id === 'TB1');
    const placement = required(required(gateway).placement);
    assert.ok('position' in placement);
    placement.position = transform(placement.position);
    // Flip the strip to keep its selected A face physically directed at the leaf.
    placement.rotationY = rotation + (side === 'A' ? 180 : 0);
    required(cfg.panelGateway).panelSide = side;
    required(cfg.panelGateway).boardSide = side === 'A' ? 'B' : 'A';
    return source;
}
for (const [side, rotation] of [['B', 90], ['A', 90], ['B', 180]] as const) {
    test(`backside space follows translated/raised leaf, yaw ${rotation}, and configured ${side}-side gateway`, () => {
        ...
        try {
            ...
            for (const open of [false, true, false]) {
                runtime.movePanel(open);
                panelBacksideGeometry(runtime, `${side}/${rotation}/${open}`);
                physicalGeometry(runtime);
            }
        }
        finally {
...
test('legal route endpoints do not permit a segment to cross a gap between backside regions', () => {
    const leaf: RoutingBox = { min: [0, 0, 0], max: [10, 10, 10] }, gateway: RoutingBox = { min: [8, 0, 12], max: [18, 10, 22] };
