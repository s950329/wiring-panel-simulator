import {required} from './fixture-types.ts';
import assert from 'node:assert/strict';
import * as T from 'three';
import type {RoutedWire} from '../../src/application/board-snapshot';
...
function intersectsBox(a: readonly number[], b: readonly number[], min: readonly number[], max: readonly number[]) {
    let first = 0, last = 1;
...
export function panelLocalPoints(runtime: ProjectRuntime, wire: RoutedWire) {
    runtime.world.updateMatrixWorld(true);
    const inverse = required(runtime.flap).matrixWorld.clone().invert().multiply(runtime.world.matrixWorld);
    return wire.points.map((point) => new T.Vector3(...point).applyMatrix4(inverse).toArray());
}
export function panelBacksideGeometry(runtime: ProjectRuntime, label = '') {
    ...
    for (const wire of runtime.routing.wires) {
        ...
        for (let i = 1; i < points.length; i++) {
            assert.equal(intersectsBox(points[i - 1], points[i], [x0, panel.thickness / 2 + wire.radius, z0], [x1, 1e9, z1]), false, `${label}: ${wire.id} segment ${i - 1} crosses the operation face`);
        }
        if (fromPanel && toPanel) {
            for (const point of points) {
                assert.ok(point[1] <= -panel.thickness / 2 - wire.radius + .002, `${label}: ${wire.id} escapes the panel backside`);
                assert.ok(point[0] >= x0 - .002 && point[0] <= x1 + .002 && point[2] >= -panel.depth - .002 && point[2] <= (runtime.panelOpen ? 5 : z1) + .002, `${label}: ${wire.id} same-panel route leaves the leaf footprint: ${point}`);
            }
        }
    }
}
export const routeLength = (wire: RoutedWire) => wire.points.slice(1).reduce((length: number, b, i: number) => length + Math.hypot(...b.map((n: number, k: number) => n - wire.points[i][k])), 0);
