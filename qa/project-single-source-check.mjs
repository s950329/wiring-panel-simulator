...
test('D1 default board has exactly one explicit phase-capable source and QF1 OFF',()=>{
 ...
 assert.equal(sources.length,1);assert.equal(sources[0].definitionId,SOURCE);assert.equal(sources[0].parameters,undefined);
 assert.equal(p.configuration.components.find(c=>c.id==='QF1').state.on,false);assert.ok(!p.configuration.components.some(c=>c.id==='CONTROL'));
});
test('P04 single source is validated by definition rather than the instance name',()=>{
 const p=single();p.configuration.components.find(c=>c.definitionId===SOURCE).id='SUPPLY_RENAMED';assert.doesNotThrow(()=>validateProject(p));
 p.configuration.components.push(external('SECOND'));assert.throws(()=>validateProject(p),/單一|一組/);
 const renamedControl=single();renamedControl.configuration.components.push(external('NOT_CALLED_CONTROL','teaching-source'));
...
test('P01 no per-source enabled/available state is accepted or serialized',()=>{
 for(const field of ['enabled','available']){const p=single();p.configuration.components.find(c=>c.definitionId===SOURCE).parameters={[field]:true};assert.throws(()=>validateProject(p),/parameters/);}
 const r=createProjectRuntime(single());try{const out=r.exportProject();assert.equal(out.configuration.components.find(c=>c.definitionId===SOURCE).parameters,undefined);
  const s=r.simulation.snapshot();assert.ok(!('power'in s));assert.ok(!('sourcePower'in s));assert.ok(!('setPower'in r.simulation));assert.ok(!('setSource'in r.simulation));
 }finally{r.dispose();}
});
test('zero-source editable drafts never receive implicit power equipment',()=>{
 const p=single();p.configuration.components=p.configuration.components.filter(c=>!c.definitionId.endsWith('source'));
...
test('A10 source is active in test mode even with QF1 OFF; real contacts alone isolate its outputs',()=>{
 const r=createProjectRuntime(single());try{
  ...
  assert.equal(net(['MAIN','L1']),net(['QF1','L1']));assert.notEqual(net(['QF1','L1']),net(['QF1','T1']));
  r.simulation.operate('QF1',{type:'toggle'});assert.equal(r.simulation.circuit().threePhaseSources[0].enabled,true);
  r.simulation.stop();assert.equal(r.components.get('QF1').state.on,true);assert.equal(r.simulation.circuit().threePhaseSources[0].enabled,false);
 }finally{r.dispose();}
});
test('generic legacy three-phase source remains explicit without new phase capability',()=>{
 const p=single();p.configuration.components.find(c=>c.definitionId===SOURCE).definitionId='teaching-three-phase-source';
 const r=createProjectRuntime(p);try{assert.equal(r.simulation.circuit().threePhaseSources[0].lineToLine,undefined);}finally{r.dispose();}
});
