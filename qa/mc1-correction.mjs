...
const scene=new T.Scene();const {world,components}=buildModel(scene);scene.updateMatrixWorld(true);
const mc=components.get('MC1'),th=components.get('TH1'),sides=mc.terminals.filter(t=>t.group==='side');
assert.equal(sides.length,8);assert.equal(mc.terminals.length,16);
for(const side of ['L','R']){
 ...
 for(const end of ['F','B']){
  ...
  assert.ok(upper.local[1]>lower.local[1]);
  assert.ok(Math.abs(lower.local[2])>Math.abs(upper.local[2]));
 }
}
assert.deepEqual(mc.terminals.filter(t=>t.group==='rear-lower').map(t=>t.id),['A1','A2']);
const isolated=buildModel(new T.Scene(),{inspectMC1:true});
assert.deepEqual([...isolated.components.keys()],['MC1','AP1','TH1']);
for(const t of mc.terminals){const other=isolated.components.get('MC1').terminals.find(q=>q.id===t.id);assert.deepEqual(other.local,t.local);}
const control=th.terminals.filter(t=>t.group==='overload-contact');
assert.deepEqual(control.map(t=>t.id).sort(),['TA','TB','TC']);
for(const obsolete of ['95','96','97','98'])assert.ok(!th.terminals.some(t=>t.id===obsolete));
...
const bounds=new T.Box3().setFromObject(th.root);
...
function reachable(comp,t){
 const target=t.hit.getWorldPosition(new T.Vector3());target.y+=1;
 let hits=0;
 for(let az=0;az<16;az++)for(const e of [.22,.45,.70,1.0,1.35,Math.PI/2]){
  ...
  if(first?.object.userData.componentId===comp.def.id&&first.object.userData.terminal===t.id)hits++;
 }
 return hits;
}
for(const [comp,list] of [[mc,[...sides,...mc.terminals.filter(t=>t.group==='rear-lower')]],[th,control]])for(const t of list){const hits=reachable(comp,t);assert.ok(hits>0,`${comp.def.id}:${t.id} is occluded from all upper-hemisphere samples`);console.log(`${comp.def.id}:${t.id} reachable ${hits}/96`);}
// Attached TH and its independent terminal geometry follow the MC assembly.
const before=byId.TA.object.getWorldPosition(new T.Vector3());mc.root.position.x+=20;scene.updateMatrixWorld(true);const after=byId.TA.object.getWorldPosition(new T.Vector3());assert.ok(Math.abs(after.x-before.x-20)<1e-7);
