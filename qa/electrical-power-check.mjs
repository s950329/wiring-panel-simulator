...
const baseModel = () => createTeachingComponent({id: 'S', definitionId: 'teaching-source'}).model;
const mainSource = id => ({id, terminals: ['L1', 'L2', 'L3'], model: baseModel()});
const phases = id => ({id, phases: ['L1', 'L2', 'L3'].map(t => ep(id, t)), profile: 'teaching-three-phase-v1', enabled: true});
function circuit() {
...
const motor = r => r.motors?.find(m => m.component === 'M1');
...
test('motor requires three distinct compatible phases and reports terminal phase order', () => {
  ...
  assert.equal(r.status, 'ok'); assert.equal(motor(r)?.state, 'powered');
  assert.deepEqual(motor(r).phaseOrder, [0, 1, 2]); assert.deepEqual(motor(r).sourceIds, ['MAIN']);
  assert.ok(has(r, 'RATING_UNVERIFIED')); assert.equal(has(r, 'SOURCE_SHORT'), false);
  for (const order of [[0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]]) {
    const reversed = {...c, wires: c.wires.map((w, i) => ({...w, from: ep('MAIN', `L${order[i] + 1}`)}))};
    assert.equal(motor(evaluateCircuit(reversed)).state, 'powered');
    assert.deepEqual(motor(evaluateCircuit(reversed)).phaseOrder, order);
  }
...
test('missing phase, duplicate phase, open motor and disabled source remain unpowered', () => {
  ...
  assert.equal(motor(missing)?.state, 'unpowered'); assert.equal(motor(missing).reason, 'missing-phase');
  assert.equal(missing.status, 'ok'); assert.ok(has(missing, 'MOTOR_MISSING_PHASE'));
  const duplicate = evaluateCircuit({...c, wires: c.wires.map((w, i) => i === 1 ? {...w, from: ep('MAIN', 'L1')} : w)});
  assert.equal(motor(duplicate).reason, 'duplicate-phase'); assert.ok(has(duplicate, 'MOTOR_DUPLICATE_PHASE'));
  assert.equal(has(duplicate, 'SOURCE_SHORT'), false);
  assert.equal(motor(evaluateCircuit({...c, wires: []})).reason, 'open');
  assert.equal(motor(evaluateCircuit({...c, threePhaseSources: [{...c.threePhaseSources[0], enabled: false}]})).reason, 'source-off');
});
...
test('motor supply from multiple sources or a mismatched profile is not accepted', () => {
  ...
  assert.equal(r.status, 'unknown'); assert.equal(motor(r).reason, 'mixed-sources');
  const incompatible = evaluateCircuit({...c, threePhaseSources: [{...c.threePhaseSources[0], profile: 'other'}]});
  assert.equal(incompatible.status, 'unknown'); assert.equal(motor(incompatible).reason, 'incompatible-supply');
});
...
test('phase shorts and cross-source ideal-rail connections stop the whole evaluation', () => {
  ...
  assert.equal(shorted.status, 'fault'); assert.ok(has(shorted, 'SOURCE_SHORT'));
  assert.equal(motor(shorted).state, 'fault');
  ...
  const control = createTeachingComponent({id: 'CONTROL', definitionId: 'teaching-source'});
  ...
  assert.equal(overlap.status, 'fault'); assert.ok(has(overlap, 'SOURCE_CONFLICT'));
});

test('a lamp in a motor phase is unsupported, not treated as a wire or an ordinary missing phase', () => {
  const c = circuit();
  const series = evaluateCircuit({...c, components: [...c.components, createTeachingComponent({id: 'HL1', definitionId: 'lamp-white'})],
    wires: [...c.wires.slice(0, 2), wire('phase-in', ['MAIN', 'L3'], ['HL1', '1']), wire('phase-out', ['HL1', '2'], ['M1', 'W'])]});
  assert.equal(series.status, 'unknown'); assert.equal(motor(series).reason, 'unsupported-series');
  assert.equal(series.loads[0].state, 'unknown'); assert.equal(has(series, 'SOURCE_SHORT'), false);
});

test('two-terminal loads on undeclared phase-to-phase supplies are explicitly unsupported', () => {
  const c = circuit(), lamp = createTeachingComponent({id: 'HL1', definitionId: 'lamp-white'});
  const r = evaluateCircuit({...c, components: [...c.components, lamp], wires: [...c.wires,
    wire('a', ['MAIN', 'L1'], ['HL1', '1']), wire('b', ['MAIN', 'L2'], ['HL1', '2'])]});
  assert.equal(r.status, 'unknown'); assert.equal(r.loads[0].state, 'unknown');
...
test('an independently powered motor cannot turn a dangling same-rail branch into a series fault', () => {
  ...
  assert.equal(before.status, 'ok'); assert.equal(motor(before).reason, 'missing-phase');
  ...
  assert.equal(r.status, 'ok'); assert.deepEqual(motor(r), motor(before));
  assert.equal(r.motors.find(m => m.component === 'M2').state, 'powered');
  assert.equal(r.loads[0].reason, 'open'); assert.equal(settleCircuit(parallel).status, 'stable');
});
...
test('load-mediated motor connections between independent sources remain open without a return loop', () => {
  ...
  const open = {...c, components: [...c.components, mainSource('SECOND'),
    ...['HL1', 'HL2'].map(id => createTeachingComponent({id, definitionId: 'lamp-white'}))],
  ...
  assert.equal(r.status, 'ok'); assert.equal(motor(r).reason, 'open');
  assert.ok(r.loads.every(l => l.reason === 'open')); assert.equal(settleCircuit(open).status, 'stable');
  // A second source-to-source branch really closes the unsupported multi-source loop.
  const closed = {...open, components: [...open.components, createTeachingComponent({id: 'HL3', definitionId: 'lamp-white'})],
  ...
  assert.equal(motor(evaluateCircuit(closed)).reason, 'unsupported-series');
...
test('two motors sharing an unsupplied terminal expose a genuine load-mediated path', () => {
  ...
  assert.equal(r.status, 'unknown'); assert.ok(r.motors.every(m => m.reason === 'unsupported-series'));
  assert.equal(has(r, 'SOURCE_SHORT'), false); assert.equal(settleCircuit(coupled).status, 'halted');
});
...
test('three-phase declarations and motor terminal IDs are validated before power is reported', () => {
  ...
  const bad = structuredClone(c); bad.components[1].model.motors[0].terminals[2] = 'missing';
  assert.ok(has(evaluateCircuit(bad), 'INVALID_ENDPOINT'));
  const repeated = structuredClone(c); repeated.components[1].model.motors[0].terminals[2] = 'U';
