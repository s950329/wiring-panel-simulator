...
function assertClear(model: ReturnType<typeof make>) {
    ...
    for (const wire of model.routing.wires) {
        ...
        assert.ok(wire.points.every((p: readonly number[]) => p[2] >= 517));
        for (const [end, point] of [[wire.from, wire.points[0]], [wire.to, wire.points.at(-1)]] as const)
            assert.ok(describeTerminal(model.world, model.components, end).anchors.some(p => Math.hypot(...p.map((v, i) => v - required(point)[i])) < .002));
    }
}
function move(model: ReturnType<typeof make>, open: boolean) {
    const previous = model.flap.rotation.x;
    model.routing.movePanel(() => { model.flap.rotation.x = open ? Math.PI : 0; model.world.updateMatrixWorld(true); }, () => { model.flap.rotation.x = previous; model.world.updateMatrixWorld(true); }, front);
}
test('different-height panel terminals try another turn instead of folding onto their own lead', () => {
    for (const reverse of [false, true]) {
        const model = make(), ends: [import('../src/electrical/contracts.ts').Endpoint,import('../src/electrical/contracts.ts').Endpoint] = [endpoint('BZ1', '2'), endpoint('HL4', '2')];
        const wire = model.routing.connect(ends[reverse ? 1 : 0], ends[reverse ? 0 : 1]);
        assertClear(model);
        for (const open of [false, true]) {
            move(model, open);
            assertClear(model);
        }
        assert.deepEqual(model.routing.wires[0].from, wire.from);
        assert.deepEqual(model.routing.wires[0].to, wire.to);
    }
});
...
test('unused TB1:42B connects to HL4:2 amid existing panel wiring without rerouting other wires', () => {
    const model = make();
    for (const [a, at, b, bt] of surrounding)
        model.routing.connect(endpoint(a, at), endpoint(b, bt));
    const before = model.routing.snapshot();
    assertClear(model);
    ...
    assert.deepEqual(model.routing.snapshot().slice(0, -1), before);
    assertClear(model);
    assert.equal(model.routing.remove(target.id), true);
    model.routing.connect(target.to, target.from);
    assertClear(model);
});
test('TB1:42B to HL4:2 keeps its shared terminal connections when the panel moves', () => {
    ...
    model.routing.connect(endpoint('TB1', '42B'), endpoint('HL4', '2'));
    assertClear(model);
