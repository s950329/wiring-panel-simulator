...
test('settling publishes matching coil and contact states, not the first transient evaluation', () => {
  ...
  assert.equal(off.status, 'stable'); assert.deepEqual(off.coils, {MC1: false});
  const on = settleCircuit(c, pressed);
  assert.equal(on.status, 'stable'); assert.deepEqual(on.coils, {MC1: true});
  assert.equal(on.iterations, 2); assert.equal(closed(on, 'MC1', 'main-1'), true);
...
test('self-hold memory depends on the wired AP contact and survives only with that path', () => {
  ...
  assert.equal(sim.step(c).coils.MC1, false);
  const started = sim.step(c, pressed); assert.equal(started.coils.MC1, true);
  // Returned data cannot overwrite the session's last stable state.
  started.coils.MC1 = false;
  assert.equal(sim.step(c).coils.MC1, true);
  assert.equal(sim.step({...c, wires: c.wires.filter(w => w.id !== 'hold-out')}).coils.MC1, false);
  assert.equal(sim.step(c).coils.MC1, false);
});

test('multiple coils update simultaneously and settle independently of array order', () => {
  const c = minimalControlCircuit();
  c.components.push(createTeachingComponent({id: 'MC2', definitionId: 'shihlin-sp16'}),
    createTeachingComponent({id: 'AP1', definitionId: 'shihlin-ap22', parentId: 'MC1'}));
  c.wires.push(wire('aux-feed', ['SUPPLY', 'L'], ['AP1', '53']), wire('second', ['AP1', '54'], ['MC2', 'A1']),
    wire('second-return', ['MC2', 'A2'], ['SUPPLY', 'N']));
  const r = settleCircuit(c, pressed);
  assert.equal(r.status, 'stable'); assert.deepEqual(r.coils, {MC1: true, MC2: true});
...
test('own NC feedback is detected as oscillation with no partial state published', () => {
  ...
  assert.equal(r.status, 'halted'); assert.equal(r.reason, 'oscillation');
  assert.equal(r.coils, null); assert.equal(r.evaluation, null);
  assert.equal(r.iterations, 2); assert.ok(r.diagnostics.some(d => d.code === 'OSCILLATION'));
});
...
test('iteration limit rejects an unverified transient result and validates its bound', () => {
  ...
  assert.equal(r.status, 'halted'); assert.equal(r.reason, 'iteration-limit'); assert.equal(r.coils, null);
  assert.equal(settleCircuit(c, pressed, {}, {maxIterations: 2}).status, 'stable');
  for (const maxIterations of [0, -1, 1.5, Infinity, NaN]) assert.throws(() => settleCircuit(c, pressed, {}, {maxIterations}), RangeError);
});
...
test('source fault halts the session until reset, which does not change wires or inputs', () => {
  ...
  assert.equal(sim.step(c, pressed).coils.MC1, true);
  const shorted = {...c, wires: [...c.wires, wire('short', ['SUPPLY', 'L'], ['SUPPLY', 'N'])]};
  const r = sim.step(shorted, pressed);
  assert.equal(r.status, 'halted'); assert.equal(r.reason, 'circuit-fault'); assert.equal(r.coils, null);
  assert.deepEqual(sim.step(c), r);
  const before = structuredClone(c); sim.reset();
  assert.equal(sim.step(c).coils.MC1, false); assert.deepEqual(c, before);
  // A physically held start input really can restart: no global anti-restart shortcut.
  sim.reset(); assert.equal(sim.step(c, pressed).coils.MC1, true);
});
...
test('invalid previous state and unsupported series halt instead of converting unknown coils to false', () => {
  ...
  assert.equal(bad.status, 'halted'); assert.ok(bad.diagnostics.some(d => d.code === 'INVALID_INPUT'));
  const series = {...c, components: [...c.components, createTeachingComponent({id: 'HL1', definitionId: 'lamp-white'})],
    wires: [...c.wires.slice(0, 2), wire('series', ['MC1', 'A2'], ['HL1', '1']), wire('return', ['HL1', '2'], ['SUPPLY', 'N'])]};
  const r = settleCircuit(series, pressed);
  assert.equal(r.status, 'halted'); assert.equal(r.reason, 'unsupported');
  assert.equal(r.evaluation, null); assert.equal(r.coils, null);
});
