...
import {HemisphereCamera} from '../src/camera.js';
globalThis.document??={createElement:()=>({getContext:()=>({fillRect(){},strokeRect(){},fillText(){}})})};
const board=JSON.parse(readFileSync(new URL('./fixtures/board-024-classroom.project.json',import.meta.url),'utf8'));
const part=(id,definitionId)=>createTeachingComponent({id,definitionId});
const same=(r,id,a,b)=>r.nets.some(n=>n.endpoints.some(e=>e.component===id&&e.terminal===a)&&n.endpoints.some(e=>e.component===id&&e.terminal===b));
...
test('all classroom button variants use same-side NO 2/3 and NC 1/4, with no diagonal conducting pair',()=>{
 for(const color of ['yellow','teal','green','red-sticker','red'])for(const pressed of [false,true]){
...
test('reset-view left/right button terminals carry the matching electrical labels without moving the original wire endpoints',()=>{
 ...
 try{
  runtime.flap.rotation.x=Math.PI;runtime.world.updateMatrixWorld(true);
  const camera=new T.PerspectiveCamera(40,1,1,5000),orbit=new HemisphereCamera(camera);orbit.preset('perspective');
  for(const id of ['PB3','PB5']){
   const terms=runtime.components.get(id).terminals.map(t=>({...t,sx:t.object.getWorldPosition(new T.Vector3()).project(camera).x}));
   const middle=terms.reduce((s,t)=>s+t.sx,0)/4;
   const left=terms.filter(t=>t.sx<middle),right=terms.filter(t=>t.sx>middle);
   assert.deepEqual(left.map(t=>t.id).sort(),['2','3']);assert.deepEqual(right.map(t=>t.id).sort(),['1','4']);
   assert.ok(left.every(t=>t.group==='NO'&&t.displayName.includes('常開')));
   assert.ok(right.every(t=>t.group==='NC'&&t.displayName.includes('常閉')));
  }
...
test('original classroom control wires start, hold, stop and trip through MC1 sides without AP1 or automatic supply',()=>{
 const runtime=createProjectRuntime(board);
 let c;
 try{c=runtime.simulation.circuit();}finally{runtime.dispose();}
 c.wires=[...board.connections.map((w,i)=>({...w,id:`original-${i}`})),...c.wires];
 for(const s of c.threePhaseSources)s.enabled=true;
 ...
 const missingSupply=run({PB3:{pressed:true}});assert.equal(missingSupply.coils.MC1,false);
 for(const phase of ['L1','L2','L3'])c.wires.push({id:`supply-${phase}`,from:{component:'MAIN',terminal:phase},to:{component:'QF1',terminal:phase}});
 const lit=(r,id)=>r.evaluation.loads.find(l=>l.component===id).state==='energized';
 ...
 for(const r of [start,hold]){assert.equal(r.coils.MC1,true);assert.equal(lit(r,'HL4'),true);assert.equal(lit(r,'HL3'),false);assert.equal(lit(r,'BZ1'),false);}
 for(const r of [idle,stop,both,trip,reset,powerOff,powerRestored]){assert.equal(r.coils.MC1,false);assert.equal(lit(r,'HL4'),false);}
...
test('classroom project imports all original control endpoints, remains stopped and drives the actual controller with its explicit supply',async()=>{
 ...
 try{
  ...
  const lit=id=>runtime.simulation.snapshot().result.evaluation.loads.find(l=>l.component===id).state==='energized';
  const operate=(id,type)=>assert.equal(runtime.simulation.operate(id,{type}).accepted,true);
  assert.equal(lit('HL4'),false);
  operate('QF1','toggle');assert.equal(lit('HL4'),false);
  operate('PB3','press');assert.equal(lit('HL4'),true);
  operate('PB3','release');assert.equal(lit('HL4'),true);
  operate('PB5','press');assert.equal(lit('HL4'),false);
  operate('PB5','release');assert.equal(lit('HL4'),false);
  operate('PB3','press');operate('PB3','release');
  operate('TH1','trip');assert.equal(lit('HL4'),false);assert.equal(lit('HL3'),true);assert.equal(lit('BZ1'),true);
  operate('TH1','reset');assert.equal(lit('HL4'),false);assert.equal(lit('HL3'),false);assert.equal(lit('BZ1'),false);
  operate('PB3','press');operate('PB3','release');operate('QF1','toggle');assert.equal(lit('HL4'),false);
  operate('QF1','toggle');assert.equal(lit('HL4'),false);
 }finally{runtime.dispose();}
});
