...
- `examples/a04-motor-start.project.json`：完整 A04 電路，28 條盤內線、6 條外接線；操作板閉合、QF1 OFF、模擬未執行。
...
頂層：`format: "wiring-panel-project"`、`schemaVersion: 1`、可選 `name`、`configuration`、`connections`。這個 schemaVersion 與網站的 WIRE-R14 不同。新版只接受白名單欄位。
