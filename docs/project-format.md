...
匯入後始終未送電。開關位置、急停鎖定、過載跳脫、旋鈕檔位、過載設定、電源可用選項及操作板開闔會還原；按住的按鈕、演示亮燈、接觸器吸合與自保持記憶不恢復。必須重新按「送電模擬」。

按「匯出配線專案 JSON」保存目前設定與實際連接；匯出不改動目前的通電或演示狀態。線號、相機、選取、電線座標、求解結果、端子局部座標及完整型號資料不進檔案。

選取任意實例後按「單獨檢視選中元件」，會顯示該實例及其組裝，不依賴 MC1 的固定名稱。檢視使用渲染圖層，不移除碰撞障礙。「載入預設盤面」則重新建立未接線的 BOARD 024，不與舊專案合併。

## 可直接載入的範例

- `examples/a04-motor-start.project.json`：完整 A04 電路，26 條盤內線、8 條外接線；操作板閉合、QF1 ON、未送電。
- `examples/board-024-empty.project.json`：原盤面配置，沒有使用者接線。
- `examples/custom-panel-less.project.json`：不同底板尺寸、兩顆元件、一條線，沒有操作板或隱含電源。
- `examples/independent-drives.project.json`：兩組不同位置／方向的 S-P16、AP-22、TH20，兩個獨立控制電源；展示同型號實例與電源選項互不共用。

A04 的啟動鍵為 PB3、停止鍵為 PB5；TH1 TEST 使 MC1 釋放、HL3 紅燈與 BZ1 警報啟動。RESET 後不自行重啟。CONTROL 與 MAIN 仍是現有的獨立教學電源，不是從真實三相 AC220V 自動取電；本輪沒有變更教學電性或實體額定值。

## 契約

頂層：`format: "wiring-panel-project"`、`schemaVersion: 1`、可選 `name`、`configuration`、`connections`。這個 schemaVersion 與網站的 WIRE-R13 不同。新版只接受白名單欄位。
...
持續輸入按型號驗證：斷路器 `state.on`；急停 `state.latched`；選擇開關 `state.position`（0、1、2）；過載 `state.trip` 與 `parameters.current`（12–18、間隔 0.5）；保險絲保護蓋 `state.open`；控制／主電源 `parameters.enabled`。缺值用該型號預設值；無效值拒絕，不截斷。瞬時 `pressed`／手動燈 `on` 即使出現在輸入中，也只驗證布林型別後丟棄，不持續演示。
...
JSON 無法攜帶任意全新型號、腳本或遠端模型。未知型號／契約版本、缺少宿主或端子、錯誤插槽與原型污染欄位均拒絕。允許保存電氣錯線練習；短路、混源或尚缺電性模型由送電求解診斷，不為了通過模擬而改接線。

資源上限：10 MB、128 元件、8,192 端子、512 使用者接線、各 64 條線槽／導軌、引用深度 16。ID 最多 100 字元（文字、數字、底線、連字號、句點，不能使用冒號或管線字元），專案名稱 200 字元。座標每軸絕對值 ≤10,000，尺寸為 ≤10,000 的有限正值；安裝中心須在對應安裝面範圍內。支援一塊矩形底板、零或一塊操作板、90° 整數倍的安裝方向，不含拖拉配置編輯器。

舊 R8–R12 整盤快照以明示舊配置規則轉換：忽略舊路線、顯示 metadata 及求解結果，但驗證型號引用、配置／元件位置一致、組裝偏移及固定銅片。舊版的固定 800×640×7 盤體才能唯一還原；不同尺寸或有矛盾的舊資料會拒絕，不猜測。舊單元件快照繼續使用 `/mc1.html` 的相容入口；一般新版專案不得把除錯快照當作輸入。
