...
使用者提供的 IMG_2689.JPG 可見同一條黑色外被電纜內的白、黑、紅三條導線接到 QF1 同側三個端子，綠線接金屬底板固定螺絲。預設盤因此明示三條 MAIN L1/L2/L3→QF1 L1/L2/L3 固定進線，與課堂由 QF1 出線端開始練習的情境一致。照片未用來判定實際相序或額定電壓；綠線不當作中性線或第四相，現有求解器不模擬保護接地故障。

供電仍依明示的單一教學來源及接點求解。固定進線只補齊上游實際配置，不會跳過 QF、保險絲、按鈕或線圈回路；原始 22 條控制線另經實際匯入、啟停、自保持、過載、斷電與復电測試。
