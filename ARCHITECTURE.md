...
`router.js` 的操作板側直連入口改用上述策略；盤內線槽和混合接線沿用既有搜尋。新路徑保留矩陣變換後的完整精度，避免四捨五入使邊界點越界。`planner.ts` 的偏好快取與 `restore.ts` 的舊路徑還原使用同一區域檢查，避免舊版碰撞合法、卻繞到操作正面的路徑漏過。正常專案仍只保存配置與電性端點，匯入時重建路徑。
