...
`wiring/panel-candidates.ts` 先比較端子 anchors 間的直角短路徑，再比較板背水平主幹與局部側向出口，以長度加轉折成本排序。變體 0～3 提供不同合法候選供群組規劃。`wiring/panel-grid.ts` 在失敗時使用有限座標網格的 weighted A*，搜尋狀態包含進入軸，保留精確 anchors、相鄰短邊與通過完整碰撞檢查的長邊；每次最多 10,000 個節點、450 ms，並接受上層共同 deadline／取消檢查。這是有界近似搜尋，不保證全域最短。耗盡候選與預算不足分別回報。
