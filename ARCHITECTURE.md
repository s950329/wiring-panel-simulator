...
`editor/catalog.ts`、`viewer.ts`、`dialog.ts` 共用既有型號與幾何工廠，提供分類卡片、靜態縮圖與單一互動 3D 預覽。`commands.ts` 產生不可變 ProjectDocument；`geometry.ts` 以實際可見模型外框驗證吸附後的位置及兩個操作板姿態。`stage.ts` 的格線與半透明預覽位於獨立暫態 root，不進入實際路由場景。

`controller.ts` 透過既有 ProjectSession 原子重建與發布，成功才推進 `history.ts` 的復原堆疊。`ui.ts` 管理選取、拖放、快捷鍵、安裝面與彈窗生命週期；建造事件不會傳給舊配線操作。相機在建造模式切為正交投影，離開時恢復原視角。JSON 格式維持 schemaVersion 1；既有元件幾何及電性未改動。

完整操作、第一版範圍及驗證說明見 `docs/custom-panel.md`。下方歷史版本中「尚未提供拖放編輯器」描述的是當時交付狀態，不是 R21 的限制。

## WIRE-R19：搜尋預算與 CI 穩定性

R18 的 GitHub CI 在 23 線刪除／反向重接後的閉合專案重建失敗。局部網格同時受 10,000 步及 450 ms 限制，會因 CPU 速度而取得不同候選；完整線組規劃又先嘗試多個相似的局部順序，較不同的全組排序排在後面，可能尚未完成便耗盡每姿態 20 秒的總預算。

`panel-grid.ts` 現在用固定 10,000 次搜尋步驟限制局部工作，時間只供診斷；每 64 步仍呼叫上層 checkpoint。`planner.ts` 在第一次遇到 `ROUTE_SEARCH_LIMIT` 時，先給穩定的全組排序一次機會，再回到既有局部候選；`ROUTE_NOT_FOUND` 仍按原本局部優先處理。保留 32 組候選、每姿態 20 秒、原子還原及全部幾何檢查。沒有加入跨姿態的失敗快取，也不以放寬碰撞或測試標準排除失敗。

CI 與本地驗證以工作流程既有 Node 22.16.0 對齊。依使用者已指定的驗收分工，CI 執行程式、實際幾何與正式／離線建置；瀏覽器／WebGL 由使用者驗收，瀏覽器測試腳本保留供手動使用。原本硬編碼 R14 的原始碼壓縮檔改用固定名稱及 verified-commit.txt 識別來源。調查及驗證見 `docs/ci-routing-budget-acceptance.md`。

## WIRE-R18：操作板背面區域與路徑成本

`wiring/panel-region.ts` 從操作板規格和當前矩陣建立板背、過門端子台走線區及操作正面禁入範圍。路徑座標仍是底板局部空間；區域驗證換算至板片局部空間，所以升高、平移及 90° 整數倍旋轉共用同一規則。完整線段必須被合法區域的聯集連續覆蓋，不能只檢查兩端點。同板接線進一步限定在板片背面；混合盤內接線保留線槽規則，但同樣禁止越過操作正面。板片規格另存 routingPanel metadata，未改變實際幾何。

`wiring/panel-candidates.ts` 先比較端子 anchors 間的直角短路徑，再比較板背水平主幹與局部側向出口，以長度加轉折成本排序。變體 0～3 提供不同合法候選供群組規劃。`wiring/panel-grid.ts` 在失敗時使用有限座標網格的 weighted A*，搜尋狀態包含進入軸，保留精確 anchors、相鄰短邊與通過完整碰撞檢查的長邊；每次最多 10,000 個搜尋步驟，並接受上層共同 deadline／取消檢查（R19 起由上層統一限制時間）。這是有界近似搜尋，不保證全域最短。耗盡候選與預算不足分別回報。

`router.ts` 的操作板側直連入口改用上述策略；盤內線槽和混合接線沿用既有搜尋。新路徑保留矩陣變換後的完整精度，避免四捨五入使邊界點越界。`planner.ts` 的偏好快取與 `restore.ts` 的舊路徑還原使用同一區域檢查，避免舊版碰撞合法、卻繞到操作正面的路徑漏過。正常專案仍只保存配置與電性端點，匯入時重建路徑。

R17 的群組規劃、兩姿態預驗證和原子提交／失敗還原保留。以下 R17、R8 等節為歷史演算法記錄；R18 的操作板側直連已不再使用世界 Y 的逐層抬高和舊橋接搜尋。驗收見 `docs/panel-backside-routing-acceptance.md`。
...
`router.ts` 的層高、線槽偏移改由兩端附近 88 scene-units 內的實際線段占用和候選變體決定。端子出口依候選輪換 anchors／側向出線；局部 A* 找到的出口可保留前段、重新驗證較高的垂直段。操作板側直連的橋接 A* 在接受完整候選前執行自交驗證，失敗會繼續搜索。`CollisionWorld` 保留造成碰撞的 wire ID；節點預算耗盡與候選空間耗盡分別記錄，不降低原有淨空。
...
| `src/views/models.ts`、`src/models/mc1.ts` | 沿用已核對的 3D 幾何建立器 |
| `src/wiring/terminals.ts` | 從穩定端點 ID 解析世界變換、盤面座標、出線方向與夾線 anchors |
| `src/wiring/router.ts` | 沿用既有線槽選擇、局部逃逸、避障與電線間距演算法 |
...
TypeScript 5.9 採 `strict: true`、`allowJs: false`、`noEmit: true`，型別檢查涵蓋 `src/`、`scripts/`、`qa/` 與 `vite.config.ts`。原有幾何、場景、主頁、路由、Node 工具與測試皆已遷移為 TypeScript；兩支 Python 瀏覽器驗收腳本改用 Node Playwright。Three.js／DOM 的動態邊界使用明確介面，測試替身與可變資料副本集中於 QA helpers，不放寬正式資料契約。遷移守門測試防止重新加入未檢查的 JS／Python、明確 `any` 或型別忽略。建置產出的 JavaScript 仍是瀏覽器執行格式，不是手寫來源。詳細命令與驗證見 [TypeScript 開發指南](docs/typescript-migration.md)。

- `npm run typecheck`：含負向型別案例，驗證不合法 action、電流值及直接改 state 會被拒絕。
- `npm test`：型別檢查、原有幾何／MC1／配線測試，以及行為、附掛連動、取消、輸入驗證與序列化。
- `npm run build`：型別檢查、獨立 HTML 匯出驗證後執行 Vite。
...
`qa/panel-route-retry-check.ts` 重現不同高度端子折返及 22 條周邊線下 42B 仍未使用卻報錯的案例，驗證雙向連接、既有線不變、淨空與面板開闔。這是實際 Three.js 幾何與路由測試，不取代瀏覽器視覺驗收。
