...
## 目前版本：WIRE-R17 操作板雙姿勢走線

新增接線先驗證展開與收合兩種姿態的完整線組，全部成功才接受接線。規劃器優先保留可行舊線，遇到衝突時協調相關電線的順序與出口候選；高度依端子附近的實際走線占用選擇，不再依盤內總線數。開闔重用已找到的兩套解，但仍核對目前實體障礙與端點位置。搜尋超限與端子被實體封住分開回報，失敗保留原盤面。

原有元件幾何、端子 ID、電性、線距及操作板側限制維持既有規則；目前驗證的是展開／收合端點姿態，不是翻轉過程的柔性線材物理。回歸納入使用者 11 線、課堂 22 線及完整 R8 23 線的逐步接線與順序變體。
